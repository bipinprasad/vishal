...
int serverConnect(char *hostname, char *port) {
...
int processServerStr(char *allstr, ssize_t allstrLen) {
	...
	do {
		...
		/* now we handle the protocol message. In this little client that just
		 means updating the prompt in the case of a login or logout, and
		 printing received IMs in a readable way */
		...
		impMsgFree(pm);
		/* start looking at next response string, just after what used to be '\n' */
		strStart = ii + 1;
		respIdx++;
...
getServerStr(void *args /* unused */) {
	...
	while (1) {
		...
		if (readBytes < 0) {
			sprintf(where, "%s: recv() failed (readBytes=%d)", me,
					(int) readBytes);
			perror(where);
			break;
		}
...
void processOp(impOp_t op, char *_args, int nargs) {
	static const char me[] = "processOp";
	char *args, name[IMP_PROTSTRING_MAXLEN + 1] = "", IM[IMP_PROTSTRING_MAXLEN+ 1] = "";
	char buff[BUFF_SIZE];
	char *str = NULL;
	impEm *iem;
	...
	if (nargs > 0) {
		...
		if (!strlen(name)) {
			fprintf(stderr, "%s: didn't get username for operation %s\n", me,
					impOpStr(op));
			return;
		}
	...
	if (nargs > 1) {
		...
		if (!strlen(IM)) {
			fprintf(stderr, "%s: didn't get IM for operation %s %s\n", me,
					impOpStr(op), name);
		}
	} else {
		IM[0] = '\0';
...
getTextInput(void *data) {
	...
	char *buff = NULL, _buff[BUFF_SIZE + 1], *cmd, *args, prompt[BUFF_SIZE + 1];
	...
	while (1) {
		...
		if (!strcmp(cmd, "register") || !strcmp(cmd, "reg")) {
			processOp(IMP_OP_REGISTER, args, 1);
		} else if (!strcmp(cmd, "login")) {
			processOp(IMP_OP_LOGIN, args, 1);
		} else if (!strcmp(cmd, "logout")) {
			processOp(IMP_OP_LOGOUT, args, 0);
		} else if (!strcmp(cmd, "friend_request") || !strcmp(cmd, "req")) {
			processOp(IMP_OP_FRIEND_REQUEST, args, 1);
		} else if (!strcmp(cmd, "friend_remove") || !strcmp(cmd, "rm")) {
			processOp(IMP_OP_FRIEND_REMOVE, args, 1);
		} else if (!strcmp(cmd, "friend_list") || !strcmp(cmd, "list")) {
			processOp(IMP_OP_FRIEND_LIST, args, 0);
		} else if (!strcmp(cmd, "im")) {
			processOp(IMP_OP_IM, args, 2);
		} else if (!strcmp(cmd, "raw")) {
			if (connfd > 0 && args) {
				if (0 > write(connfd, args, strlen(args))) {
...
int textInit(char *host, char *port) {
...
int main(int argc, char *argv[]) {
	...
	if (textInit(server, port)) {
		fprintf(stderr, "%s: problem initializing\n", me);
		exit(1);
	}
	exit(0);
}
